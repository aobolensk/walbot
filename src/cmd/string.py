...
class StringCommands(BaseCmd):
    ...
    async def _emojify(self, cmd_line: List[str], execution_ctx: ExecutionContext) -> str:
        """Emojify text
    ...
    async def _demojify(self, cmd_line: List[str], execution_ctx: ExecutionContext) -> str:
        """Demojify text
    Example: !demojify 🇭 🇪 🇱 🇱 🇴"""
        if not await Command.check_args_count(execution_ctx, cmd_line, min=2):
            return
